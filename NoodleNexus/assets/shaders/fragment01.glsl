...
//	vec3 fvertexNormal = vec3(0.0f, 1.0f, 0.0f);
	...
	//
	//	// Apply flicker AFTER setting the final color
	//	if (bIsMovingUV) {
	//		float flicker = 0.5 + 0.5 * sin(time * 5.0); // Fast flickering
	//		finalPixelColour = finalPixelColour * flicker;
	//	}
	//
	//	return;
	//}
	...
	if (bDoNotLight)
	{
		...
		if(bIsMovingUV)
		{
			// Flickering effect (randomized for more variation)
			float flicker = 0.3 + 0.7 * sin(time * 0.4 + sin(time * flickerValue)); // Uneven flickering

			// Static noise effect (pseudo-random grain)
			float noise = fract(sin(dot(gl_FragCoord.xy, vec2(12.9898, 78.233))) * 43758.5453);
			noise = mix(0.8, 1.2, noise); // Subtle contrast variation

			// Apply the flicker and noise to the final color
			finalPixelColour.r -= flicker * noise;
			finalPixelColour.g *= flicker / noise;
			finalPixelColour.b *= flicker + noise;
		}
