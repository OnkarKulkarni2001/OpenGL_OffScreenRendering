...
void DrawMesh(sMesh* pCurMesh, GLuint program, bool SetTexturesFromMeshInfo = true)
{
    ...
    GLint flickerValueLoc = glGetUniformLocation(program, "flickerValue");

    GLint bIsMovingUV_Loc = glGetUniformLocation(program, "bIsMovingUV");
    if (pCurMesh->bIsMovingUV) {
        glUniform1f(bIsMovingUV_Loc, (GLfloat)GL_TRUE);
        glUniform1f(flickerValueLoc, pCurMesh->flickerValue);
    }
    else {
        glUniform1f(bIsMovingUV_Loc, (GLfloat)GL_FALSE);
    }
